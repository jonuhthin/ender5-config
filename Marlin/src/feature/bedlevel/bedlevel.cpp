...
void reset_bed_level() {
  ...
  void _manual_goto_xy(const xy_pos_t &pos) {
    ...
    // Get the resting Z position for after the XY move
      ...
      float finalz = _MAX(0, MANUAL_PROBE_START_Z); // If a MANUAL_PROBE_START_Z value is set, always respect it
      if (bedlevel.mesh_is_valid()) {
        finalz = bedlevel.get_z_correction(pos);
      }
      #warning "using existing mesh value when available"
    #else
